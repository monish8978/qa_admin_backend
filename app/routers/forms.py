...
def _serialize(form) -> dict:
    return {
        "id": form.id,
        "formKey": form.formKey,
        "version": form.version,
        "departmentId": getattr(form, "departmentId", None),
        "name": form.name,
...
def list_forms(
    ...
    with _tenant_session(payload) as ts:
        ...
        serialized = [_serialize(r) for r in paginated_rows]
        res = {
            "items": serialized,
            "pagination": {
                "page": page,
...
def create_form(
    ...
    with _tenant_session(payload) as ts:
        ...
        return build_response(_serialize(form), rid)
